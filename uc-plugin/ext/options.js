...
uc.KeyManagement = new function () {
    ...
    function changeEditKeyColor(newColor) {
        var keyIcon = $("#edit-key-title").find(".uc-key");
        keyIcon.removeClass("uc-color-" + activeEditKeyColor);
        activeEditKeyColor = newColor;
        keyIcon.addClass("uc-color-" + activeEditKeyColor);
    }
    ...
    this.initialize = function (domains, newKeyCallback, editKeyCallback, deleteKeyCallback, exportKeyFunction, importKeyFunction, defaultChangedCallback) {
        ...
        function createKeyUI(domain, key, defaultKey, url) {
            var keyButton = $("<div class='key-holder" + (defaultKey ? " default-key" : "") +
                "'><div class='uc-key uc-color-" + key.color +
                "'></div><div class='key-name-wrapper'><div class='uc-key-name uc-color-" + key.color +
            ...
            keyButton.click(function () {
                ...
                displayKeyDialog("edit-key", function() {
                    ...
                    // Update the UI.
                    var label = keyButton.find(".uc-key-name");
                    keyButton.find(".uc-key").add(label).removeClass("uc-color-" + key.color).addClass("uc-color-" + newColor);
                    label.text(newName);
