...
Content.within = function () {
	...
	function onShimProp(e) {
		...
		Object.defineProperty(node, name, {
			...
			set: function (v) {
				value = v;
				var event = new CustomEvent('uc-prop-set-' + name, {detail: value});
				node.dispatchEvent(event);
			}
		});
		node.addEventListener('uc-set-prop-' + name, function (e) {
			value = e.detail;
		});
	}
	...
	function onShimMethod(e) {
		...
		var name = e.detail.name;
		node[name] = function () {
			var event = new Event('uc-method-call-' + name);
			node.dispatchEvent(event);
		};
	}
	...
	function setup(win) {
		...
		// configuring ethereum client
		web3.setProvider(new web3.providers.HttpProvider('http://localhost:8545'));
		
		win.addEventListener('uc-add-listeners', onSetup, true);
		win.addEventListener('uc-shim-prop', onShimProp, true);
		win.addEventListener('uc-shim-method', onShimMethod, true);
		win.addEventListener('uc-delete-prop', onDeleteProp, true);
...
Content.propagate = function (iframe) {
	var event = new Event('uc-add-listeners', false, false);
	iframe.dispatchEvent(event);
};

Content.shimProp = function (node, name, value, handler) {
	node.addEventListener('uc-prop-set-' + name, function (e) {
		handler(e.detail);
	});
	var event = new CustomEvent('uc-shim-prop', {detail: {name: name, value: value}});
	node.dispatchEvent(event);
	return function (v) {
		var event = new CustomEvent('uc-set-prop-' + name, {detail: v});
		node.dispatchEvent(event);
	};
};

Content.shimMethod = function (node, name, handler) {
	node.addEventListener('uc-method-call-' + name, function (e) {
		handler();
	});
	var event = new CustomEvent('uc-shim-method', {detail: {name: name}});
	node.dispatchEvent(event);
};

Content.deleteProp = function (node, name) {
	var event = new CustomEvent('uc-delete-prop', {detail: {name: name}});
	node.dispatchEvent(event);
};
...
var Codec = {
	CODE_PATTERN: /=\?uc-(\w*)\?([A-Za-z0-9+\/=]*)\?([^?\s]*)\?=/g
...
Codec.encode = function (mode, fingerprint, message) {
	...
	if (mode.queryOnly) {
		...
		return '=?uc-' + fingerprint + '?' + data + '?' + tags + '?=';
	}
};
...
Rewriter.checkBlacklist = function (node) {
	...
	if (node.nodeType === Document.ELEMENT_NODE) {
		if ('ucReplaced' in node) return true;
		if ('ucUpdateContent' in node) return true;
...
Widgets.Delegated.prototype.activateDelegate = function () {
	...
	shadowHost.ucInputEarly = this.onInputEarly.bind(this);
	shadowHost.ucHandlePrivateEvent = this.handlePrivateEvent.bind(this);
...
Widgets.adapters.ContentEditable = function (e, o) {
	Widgets.KeyChanger.call(this, e, o);
	this.node.ucUpdateContent = this.updateContent.bind(this);
...
Widgets.onInputEarly = function (e) {
	if ('ucInputEarly' in e.target) {
		e.target.ucInputEarly(e);
	}
};

Widgets.onInterceptKey = function (e) {
	if ('ucHandlePrivateEvent' in e.target) {
		var shouldStop = e.target.ucHandlePrivateEvent(e);
...
Widgets.updateContent = function (node) {
	if (!node) return;
	if (node.nodeType === Document.ELEMENT_NODE) {
		if ('ucUpdateContent' in node) {
			node.ucUpdateContent();
			return;
		}
	}
